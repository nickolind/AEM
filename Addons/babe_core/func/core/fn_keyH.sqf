...
for "_i" from 0 to (_count_mods - 1) do
{
	...
	for "_k" from 0 to (_count_keys - 1) do
	{
		...
		_pars = (getarray (configfile >> "CfgModSettings" >> _mod >> "keybinds" >> "b" + (str (_k + 1)))) select 3;
		
		_keyinfo = [_modkeys, _fnc, _pars];
...
if (str _keys != "[1]") then
{
	call compile (format ["%1 call %2", _pars, _fnc]);
}
